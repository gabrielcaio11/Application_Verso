...
class ArticleControllerTest {
  ...
  void setUp() {
    ...
    articleResponse =
        new ArticleResponseWithTitleAndStatusAndCategoryName(
            "Test Article Title Valid",
            "This is a test content with more than 10 characters for validation",
            "Technology");
  ...
  void create_ShouldReturnCreatedArticle() throws Exception {
    ...
    mockMvc
        ...
        .andExpect(
            jsonPath("$.content")
                .value("This is a test content with more than 10 characters for validation"))
  ...
  void create_ShouldReturnUnprocessableEntity_WhenTitleIsBlank() throws Exception {
    CreateArticleRequestDTO invalidRequest =
        new CreateArticleRequestDTO(
            "", // título vazio
    ...
    mockMvc
        ...
        .andExpect(
            jsonPath("$.errors[0].message")
                .exists()); // Apenas verifica que há uma mensagem para o campo title
  ...
  void create_ShouldReturnUnprocessableEntity_WhenContentIsTooShort() throws Exception {
    ...
    mockMvc
        ...
        .andExpect(
            jsonPath("$.errors[0].message").value("Conteúdo deve ter pelo menos 10 caracteres"));
  ...
  void findAllPublished_ShouldReturnPageOfArticles() throws Exception {
    Page<ArticleResponseWithTitleAndStatusAndCategoryName> page =
        new PageImpl<>(List.of(articleResponse), PageRequest.of(0, 10), 1);

    when(articleService.findAllArticlesPublicados(any(Pageable.class))).thenReturn(page);
  ...
  void findAllDrafts_ShouldReturnPageOfDrafts() throws Exception {
    ...
    when(articleService.findAllArticlesRascunho(any(Pageable.class))).thenReturn(page);

    mockMvc
        .perform(
            get("/verso/articles/drafts")
                .param("page", "0")
                .param("size", "10")
                .param("sort", "createdAt,desc"))
        .andExpect(status().isOk())
  ...
  void findById_ShouldReturnArticle() throws Exception {
    ...
    mockMvc
        .perform(get("/verso/articles/{id}", articleId))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.title").value("Test Article Title Valid"))
        .andExpect(
            jsonPath("$.content")
  ...
  void update_ShouldReturnUpdatedArticle() throws Exception {
    ...
    mockMvc
        .perform(
            put("/verso/articles/{id}", articleId)
                .with(csrf())
                .contentType(MediaType.APPLICATION_JSON)
  ...
  void update_ShouldReturnUnprocessableEntity_WhenContentIsTooShort() throws Exception {
    Long articleId = 1L;
    UpdateArticleRequestDTO invalidRequest =
        new UpdateArticleRequestDTO(
            "Valid Title",
            "Short", // conteúdo muito curto
            "Technology",
  ...
  void update_ShouldReturnUnprocessableEntity_WhenTitleIsTooShort() throws Exception {
    ...
    mockMvc
        ...
        .andExpect(jsonPath("$.errors[0].fieldName").value("title"))
        .andExpect(
            jsonPath("$.errors[0].message").value("Título deve ter entre 3 e 200 caracteres"));
  ...
  void delete_ShouldReturnNoContent() throws Exception {
    ...
    mockMvc
        .perform(delete("/verso/articles/{id}", articleId).with(csrf()))
  ...
  void findAllPublished_ShouldReturnUnauthorized_WhenNotAuthenticated() throws Exception {
    mockMvc.perform(get("/verso/articles")).andExpect(status().isUnauthorized());
  ...
  void findAllDrafts_ShouldReturnUnauthorized_WhenNotAuthenticated() throws Exception {
    mockMvc.perform(get("/verso/articles/drafts")).andExpect(status().isUnauthorized());
  ...
  void findById_ShouldReturnUnauthorized_WhenNotAuthenticated() throws Exception {
    mockMvc.perform(get("/verso/articles/1")).andExpect(status().isUnauthorized());
  ...
  void delete_ShouldReturnUnauthorized_WhenNotAuthenticated() throws Exception {
    mockMvc.perform(delete("/verso/articles/1").with(csrf())).andExpect(status().isUnauthorized());
  ...
  void create_ShouldValidateMinimumValidData() throws Exception {
    ...
    CreateArticleRequestDTO minimalValidRequest =
        new CreateArticleRequestDTO(
    ...
    CreateArticleResponseDTO minimalResponse =
        new CreateArticleResponseDTO("ABC", "Exactly10c", "Cat");

    when(articleService.create(any(CreateArticleRequestDTO.class))).thenReturn(minimalResponse);

...
public class FollowService {
    ...
    public FollowResponseDTO followUser(Long userId) {
        ...
        log.info("[FOLLOW] Tentando seguir usuário. followerId={}, followingId={}",
        ...
        if (followRepository.existsByFollowerAndFollowing(currentUser, userToFollow)) {
            log.warn("[FOLLOW] Usuário já estava seguindo. followerId={}, followingId={}",
                    currentUser.getId(), userId);
            throw new BusinessException("Você já está seguindo este usuário");
        }

        // Criar seguimento
        log.debug("[FOLLOW] Criando relação de follow no banco. followerId={}, followingId={}",
                currentUser.getId(), userId);
        ...
        log.info("[FOLLOW] Usuário seguido com sucesso. followId={}, followerId={}, followingId={}",
                follow.getId(), currentUser.getId(), userId);
    ...
    public void unfollowUser(Long userId) {
        ...
        log.info("[FOLLOW] Tentando deixar de seguir usuário. followerId={}, followingId={}",
                currentUser.getId(), userId);

        var userToUnfollow = userRepository.findById(userId)
                .orElseThrow(() -> {
                    log.warn("[FOLLOW] Usuário para deixar de seguir não encontrado. followingId={}", userId);
                    return new ResourceNotFoundException("Usuário não encontrado");
                });

        if (!followRepository.existsByFollowerAndFollowing(currentUser, userToUnfollow)) {
            log.warn("[FOLLOW] Tentativa de desfazer follow inexistente. followerId={}, followingId={}",
                    currentUser.getId(), userId);
        ...
        log.info("[FOLLOW] Follow removido com sucesso. followerId={}, unfollowedId={}",
                currentUser.getId(), userId);
    }
    ...
    public Page<UserProfileDTO> getFollowing(Pageable pageable) {
        ...
        log.debug("[FOLLOW] Buscando usuários que o currentUser segue. followerId={}, page={}",
                currentUser.getId(), pageable);

        var followingPage = followRepository.findFollowingByFollower(currentUser, pageable);

        log.info("[FOLLOW] Following recuperado. followerId={}, totalElements={}",
                currentUser.getId(), followingPage.getTotalElements());
    ...
    public Page<UserProfileDTO> getFollowers(Pageable pageable) {
        ...
        log.debug("[FOLLOW] Buscando seguidores do currentUser. followingId={}, page={}",
                currentUser.getId(), pageable);

        var followersPage = followRepository.findFollowersByFollowing(currentUser, pageable);

        log.info("[FOLLOW] Followers recuperados. followingId={}, totalElements={}",
                currentUser.getId(), followersPage.getTotalElements());
    ...
    public UserProfileDTO getUserProfile(Long userId) {
        ...
        log.info("[PROFILE] Buscando perfil do usuário. targetUserId={}, requesterId={}",
    ...
    public boolean isFollowing(Long userId) {
        ...
        log.debug("[FOLLOW] Verificando isFollowing. followerId={}, targetUserId={}",
                currentUser.getId(), userId);

        var user = userRepository.findById(userId)
                .orElseThrow(() -> {
                    log.warn("[FOLLOW] Usuário não encontrado para verificação de follow. userId={}", userId);
                    return new ResourceNotFoundException("Usuário não encontrado");
                });

        boolean result = followRepository.existsByFollowerAndFollowing(currentUser, user);

        log.debug("[FOLLOW] isFollowing result. followerId={}, targetUserId={}, result={}",
                currentUser.getId(), userId, result);

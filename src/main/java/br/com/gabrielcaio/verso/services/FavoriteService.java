...
public class FavoriteService {
    ...
    public FavoriteResponseDTO addFavorite(Long articleId) {
        ...
        log.info("[FAVORITE] Iniciando processo para favoritar artigo. articleId={}, userId={}",
                articleId, currentUser.getId());

        var article = articleRepository.findById(articleId)
                .orElseThrow(() -> {
                    log.warn("[FAVORITE] Artigo não encontrado ao tentar favoritar. articleId={}, userId={}",
                            articleId, currentUser.getId());
                    return new ResourceNotFoundException("Artigo não encontrado");
                });

        if (article.getStatus() != ArticleStatus.PUBLICADO) {
            log.warn("[FAVORITE] Tentativa de favoritar artigo NÃO publicado. articleId={}, status={}, userId={}",
                    articleId, article.getStatus(), currentUser.getId());
        ...
        if (favoriteRepository.existsByUserAndArticleId(currentUser, articleId)) {
            log.warn("[FAVORITE] Artigo já estava favoritado anteriormente. articleId={}, userId={}",
                    articleId, currentUser.getId());
            throw new BusinessException("Este artigo já está nos seus favoritos");
        }

        // Criar favorito
        log.debug("[FAVORITE] Criando novo favorito no banco. articleId={}, userId={}",
                articleId, currentUser.getId());
        ...
        log.info("[FAVORITE] Artigo favoritado com sucesso. favoriteId={}, articleId={}, userId={}",
                favorite.getId(), articleId, currentUser.getId());
    ...
    public void removeFavorite(Long articleId) {
        var currentUser = userService.getCurrentUser();
        log.info("[FAVORITE] Iniciando remoção de favorito. articleId={}, userId={}",
                articleId, currentUser.getId());

        if (!favoriteRepository.existsByUserAndArticleId(currentUser, articleId)) {
            log.warn("[FAVORITE] Tentativa de remover favorito inexistente. articleId={}, userId={}",
                    articleId, currentUser.getId());
        ...
        log.info("[FAVORITE] Favorito removido com sucesso. articleId={}, userId={}",
                articleId, currentUser.getId());
    }
    ...
    public Page<FavoriteResponseDTO> findAllFavorites(Pageable pageable) {
        ...
        log.info("[FAVORITE] Favoritos recuperados com sucesso. userId={}, totalElements={}",
                currentUser.getId(), favoritesPage.getTotalElements());
    ...
    public boolean isFavorite(Long articleId) {
        ...
        log.debug("[FAVORITE] isFavorite verificado. articleId={}, userId={}, result={}",
                articleId, currentUser.getId(), result);

...
public class ArticleService {
    ...
    public Page<ArticleResponseWithTitleAndStatusAndCategoryName> findAllArticlesPublicados(Pageable pageable) {
        log.info("Buscando artigos PUBLICADOS com paginação page={} size={}", pageable.getPageNumber(), pageable.getPageSize());
    ...
    @Transactional(readOnly = true)
    public Page<ArticleResponseWithTitleAndStatusAndCategoryName> findAllArticlesRascunho(Pageable pageable) {

        var currentUser = userService.getCurrentUser();
        log.info("Buscando rascunhos do usuário id={} username={}", currentUser.getId(), currentUser.getUsername());

        var articlesPage = articleRepository.findAllByStatusAndAuthor(ArticleStatus.RASCUNHO, currentUser, pageable);

        log.info("Encontrados {} artigos rascunho do usuário {}", articlesPage.getTotalElements(), currentUser.getUsername());
        return articlesPage.map(articleMapper::toResponseWithTitleAndStatusAndCategoryName);
    }
    ...
    public ArticleResponseWithTitleAndStatusAndCategoryName findById(Long id) {
        ...
        log.debug("Artigo id={} é RASCUNHO, verificando autorização. Autor={}, Usuário Atual={}",
                id, article.getAuthor().getId(), currentUser.getId());
    ...
    public ArticleResponseWithTitleAndStatusAndCategoryName update(Long id, UpdateArticleRequestDTO updated) {
        ...
        if (!updated.getCategory().isBlank()) {
            var categoryName = updated.getCategory().toUpperCase();
    ...
    public void delete(Long id) {
        ...
        var currentUser = userService.getCurrentUser();
        log.debug("Validando permissão para excluir artigo id={} do usuário {}", id, currentUser.getUsername());
    ...
    public CreateArticleResponseDTO create(CreateArticleRequestDTO dto) {
        ...
        String categoryName = dto.getCategory().trim().toUpperCase();
    ...
    private ArticleStatus parseStatus(String rawStatus) {
        try {
            return ArticleStatus.valueOf(rawStatus.trim().toUpperCase());
        } catch (Exception e) {

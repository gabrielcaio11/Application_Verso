...
public class RolesService {
    ...
    public RolesWithIdAndName save(CreateRolesRequestDTO dto) {
        ...
        log.info("[RolesService] Role criada com sucesso: {} - {}", savedRole.getId(), savedRole.getName());
    ...
    public RolesWithIdAndName update(Long id, CreateRolesRequestDTO dto) {
        ...
        Roles existingRole = rolesRepository.findById(id)
                .orElseThrow(() -> {
                    log.warn("[RolesService] Falha ao atualizar. Role não encontrada para ID: {}", id);
                    return new ResourceNotFoundException("Role não encontrada com id: " + id);
                });
        ...
        log.info("[RolesService] Role atualizada com sucesso: {} - {}", existingRole.getId(), existingRole.getName());
    ...
    public void delete(Long id) {
        ...
        if (userService.findUsersByRole(role).isPresent()) {
            log.warn("[RolesService] Não é possível excluir role ID: {}. Existem usuários associados.", id);

...
public class UserService {
    ...
    public void register(UserDTO dto) {
        ...
        log.info("[UserService] Username gerado a partir do email '{}': {}", dto.getEmail(), usernameGerado);
        ...
        log.info("[UserService] Usuário registrado com sucesso: {} (email: {})",
    ...
    public Page<String> findAll(Pageable pageable) {
        ...
        log.info("[UserService] Total de usuários retornados na página: {}", page.getNumberOfElements());
    ...
    private void addRoles(User user, UserDTO dto) {
        ...
        roles.stream()
                ...
                .forEach(roleName -> {
                    log.info("[UserService] Atribuindo role '{}' ao usuário {}", roleName, user.getUsername());

                    Roles role = rolesRepository.findByName(roleName)
                            .orElseThrow(() -> {
                                log.error("[UserService] Role '{}' não encontrada ao registrar usuário {}", roleName, user.getUsername());
    ...
    private String obterUserNamePeloEmail(String email) {
        ...
        String usernameFinal =
                local.length() > MAX_LENGTH ? local.substring(0, MAX_LENGTH) : local;

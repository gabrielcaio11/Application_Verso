...
public class CommentService
{
    ...
    public CommentResponseDTO create(Long articleId, CreateCommentRequestDTO dto)
    {
        ...
        if(dto.getParentId() != null)
        {
            ...
            parent = commentRepository.findById(dto.getParentId())
                    ...
                    {
                        log.warn(
                                "[COMMENT CREATE] Comentário pai {} não encontrado",
                                dto.getParentId()
                        );
                        return new ResourceNotFoundException("Comentário pai não encontrado");
                    });

            if(!parent.getArticle()
                    .getId()
                    .equals(articleId))
            {
                log.warn(
                        "[COMMENT CREATE] Comentário pai {} pertence a outro artigo",
                        dto.getParentId()
                );
        ...
        try
        {
            ...
            {
                log.info(
                        "[COMMENT CREATE] Criando notificação para autor do artigo {}",
                        article.getId()
                );
                notificationService.createNotificationForArticleComment(article, comment);
            } else if(!parent.getAuthor()
                    .getId()
                    .equals(author.getId()))
            {
                log.info(
    ...
    public Page<ThreadedCommentDTO> listThreadedByArticle(Long articleId, Pageable pageable)
    {
        ...
        var roots = commentRepository.findByArticleAndParentIsNullOrderByCreatedAtAsc(
                article, pageable);
        ...
        return roots.map(root -> new ThreadedCommentDTO(
                ...
                root.getArticle()
                        .getId(),
                root.getCreatedAt(),
                commentRepository.findByParentOrderByCreatedAtAsc(root)
                        .stream()
                        .map(child -> new ThreadedCommentDTO(
                                ...
                                child.getArticle()
                                        .getId(),
                                child.getCreatedAt(),
                                commentRepository.findByParentOrderByCreatedAtAsc(child)
                                        .stream()
    ...
    public void delete(Long commentId)
    {
        ...
        var isAuthor = comment.getAuthor()
                .getId()
                .equals(currentUser.getId());
        var isArticleAuthor = comment.getArticle()
                .getAuthor()
                .getId()
                .equals(currentUser.getId());
